...
/*   Updated: 2024/04/29 23:14:17 by emduncan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
- Looping and Searching
    - The function uses a while loop with two conditions:
    	- n-- > 0: This pre-decrement loop condition ensures n is decremented
			before each iteration and the loop continues as long as n is greater
			than 0. This effectively sets the loop to iterate n times.
    	- *point == chr: This condition checks if the current byte pointed to by
			*point is equal to the character chr we're searching for.

- Returning on Match:
    - Inside the loop:
        - If the condition *point == chr is true, it means we found the character
			chr at the current memory location pointed to by point.
            - In this case, the function immediately returns a pointer cast to
				void*: ((void *)point). This pointer points to the location where
				the matching character was found in the memory block.

- Moving the Pointer:
    - If the character is not found (*point != chr), the loop continues:
        - point++: The pointer point is incremented by 1. This moves it to the
			next byte in the memory block for the next iteration.
